...
class OrderDetailsScreen extends StatelessWidget {
  ...
  final Order order;
  OrderDetailsScreen(
      {super.key, required this.order, this.fromNotification = false});
  ...
  Widget _storeCardWidget(context) {
    return Padding(
        padding: const EdgeInsets.symmetric(horizontal: 17, vertical: 10),
        child: Column(
            children: order.stores.asMap().entries.map<Widget>((entry) {
          var store = entry.value;
          return Padding(
            padding: EdgeInsets.only(bottom: 10.0),
            child: ElevatedButton(
              style: AppStyle.secElevatedBtnStyle,
              onPressed: () {},
              child: Column(
                children: [
                  Row(
                    ...
                    children: <Widget>[
                      ...
                      Padding(
                        padding:
                            EdgeInsets.only(left: ScreenUtil().setWidth(12.7)),
                        child: Text(
                          store.name,
                      ...
                      children: [
                        Column(
                          ...
                          children: store.items.map<Widget>((item) {
                            return SizedBox(
                              width: ScreenUtil.defaultSize.width - 150,
                              child: Row(
                                children: [
                                  Text(
                                    item.title,
                        ...
                        ElevatedButton(
                            onPressed: () async {
                              final navigator = Navigator.of(context);
                              if (store.storeStatus != "done") {
                                await AuthService().updateGenieProgress(
                                    orderId: order.orderId,
                                    step: 'goToStore',
                                    storeIndex: entry.key);
                                Message message = Message(
                                    senderId: order.genieId,
                                    receiverId: order.customerId,
                                    text:
                                        'The genie is currently checking the ${store.name} for the items you requested.');
                                SocketService().sendMessage(
                                    chatId: ChatId, message: message);
                                  ...
                                  child: Container(
                                    ...
                                    decoration: BoxDecoration(
                                      borderRadius: BorderRadius.all(
                                          Radius.elliptical(20.72, 20.72)),
                                      color: const Color(0xfff7f7f7),
                                      border: Border.all(
                                          width: 3.0,
                                          color: Color(
                                              store.storeStatus == "done"
                                                  ? 0xff203b4c
                                                  : 0xffed1b24)),
                                    ),
                                  ),
